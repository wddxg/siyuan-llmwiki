...
2. **CLI 版本 ≥ 1.2.3**：运行 `youdaonote version` 确认版本，若 CLI 支持 `upgrade` 命令则优先使用 `youdaonote upgrade` 升级；否则参考安装指南（https://note.youdao.com/help-center/cli-install-guide.html）
3. **文件夹结构由 Agent 自动创建**：初始化时 Agent 会自动执行 `youdaonote mkdir` 建立以下结构：
